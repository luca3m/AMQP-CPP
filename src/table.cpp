...
std::string Table::getString(const std::string& field_name) const
{
...
Array Table::getArray(const std::string& field_name) const
{
    const Field& field = get(field_name);
    if (field.typeID() == 'A')
    {
        return dynamic_cast<const Array&>(field);
    }
    return Array();
}
